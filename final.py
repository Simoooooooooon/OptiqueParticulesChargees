...
class MyWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def connect_to_gpp_2323(self):
        try:
            ...
            rm = pyvisa.ResourceManager()
            items = rm.list_resources()  # Lists the connected VISA devices
            if (not self.comboBox_gpp_2323.currentText() in items) or (self.comboBox_gpp_2323.currentText() == ""):  # Checks if the device is connected
    ...
    def connect_to_gpp_4323(self):
        try:
            ...
            rm = pyvisa.ResourceManager()
            items = rm.list_resources()  # Lists the connected VISA devices
            if (not self.comboBox_gpp_4323.currentText() in items) or (self.comboBox_gpp_4323.currentText() == ""):  # Checks if the device is connected
                QtWidgets.QMessageBox.information(self, 'Error', 'Wrong choice')
                self.populate_gpp_4323_combobox()
                self.port_gpp_4323 = None
            else:  # If the device is connected, we do this
    ...
    def gpp_2323_slider_changed(self):
        try:
            ...
            self.label_current_tension_gpp_2323.setText(f'Supply voltage (V) : {str(tension)}')   # Shows to the user the current tension
            if tension >= 0:
                self.gpp_2323.write(f'VSET1:{tension}')
                self.gpp_2323.write(f'VSET2:0')
            else:
                self.gpp_2323.write(f'VSET1:0')
                self.gpp_2323.write(f'VSET2:{-tension}')
        except Exception as e:
    ...
    def gpp_4323_tension_slider_changed(self):
        try:
            ...
            self.label_current_tension_gpp_4323.setText(f'Supply voltage (V) : {str(tension)}')   # Shows to the user the current tension
            if tension >= 0:
                self.gpp_4323.write(f'VSET1:{tension}')
                self.gpp_4323.write(f'VSET2:0')
            else:
                self.gpp_4323.write(f'VSET1:0')
                self.gpp_4323.write(f'VSET2:{-tension}')
        except Exception as e:
