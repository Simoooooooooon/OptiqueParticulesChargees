...
class Window(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        self.pushButton_connect.clicked.connect(self.port_connect) #Connect a function to a clicked element on the GUI          
    ...
    def port_connect(self):
        ...
        try : 
            self.ser = serial.Serial(port=self.port_com, baudrate=57600, bytesize=8, timeout=0.5, stopbits=1) #Open and configure the port
    ...
    def proofreading (self):        
        ...
        self.port.emit(self.ser) #Create a signal that is emitted and can be read in the thread. Here, the signal is the status of the port.        
        self.worker.data.connect(self.actual_values)
                
        #Graph1 for energy current
        
        pen1=pg.mkPen(color=(255, 0, 0)) #Color of the line
        self.data_line1 = self.graph_energy_current.plot(self.Y1, pen=pen1) #Graph initialization. This graph is empty in the beginning.
...
class ThreadWorker(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            #Energy control
            self.rs.write(b'>1,2\r') #CMD_GET_STATUS for Energy (Jumper 1). Allows the proofreading of the values sent by the card.
